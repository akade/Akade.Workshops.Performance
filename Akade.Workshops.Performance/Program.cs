...
async Task EnsureDBCreatedAsync(WebApplication app)
{
    ...
    _ = await scope.ServiceProvider.GetRequiredService<HistoricalWeatherDataContext>().Database.EnsureCreatedAsync();
}

async Task EnsureDBDeletedAsync(WebApplication app)
{
    using IServiceScope scope = app.Services.CreateScope();
    _ = await scope.ServiceProvider.GetRequiredService<HistoricalWeatherDataContext>().Database.EnsureDeletedAsync();
}
